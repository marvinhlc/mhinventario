...
{
    public partial class DocumentosEntradasEditor : Estilos.FormMantenimiento
    {
        ...
        public DocumentosEntradasEditor(int _iddocumento = 0)
        {
        ...
        private void dgvCatalogo_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                ...
                int _idproducto = _row.Cells["IDProducto"].Value.TextoToEntero();
                ModelosManager.CLS.ProductoNombres _elproducto = new ModelosManager.CLS.ProductoNombres(_idproducto);

                _cantidad = _row.Cells["Cantidad"].Value.TextoToDouble();
                _precio = _row.Cells["Precio"].Value.TextoToDouble();
                _total = (_cantidad * _precio);
                _costo = _elproducto.UltimoCosto.Valor.TextoToDouble();
                ...
                //    if (_costo != _precio)
                //    {
                //        DocumentosEntradasPrecios _precios = new DocumentosEntradasPrecios(_elproducto);
            ...
            try
            {
                ...
                if (dgvCatalogo.ExistenFilasEditadas("Cantidad") > 0)
                {
                    if (_documento.Existe)
                    {
                        ...
                        DocumentosEntradasGuardar _guardar = new DocumentosEntradasGuardar(_documento);
                    ...
                    {
                        int _idpersona = cbbProveedores.ComboBox.SelectedValue.TextoToEntero();
                        DocumentosEntradasGuardar _guardar = new DocumentosEntradasGuardar(_idpersona);
                        _guardar.doRefrescarFormularioExterno += Guardar;
                        _guardar.ShowDialog(this);
                        if (_guardar.Ok)
                        {
