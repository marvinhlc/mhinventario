...
namespace VistasManager.UI
{
    public partial class ReporteCaja : Form
    {
        ...
        private void GenerarReporte(string _pformato = "VP")
        {
            ...
            string _estacion = txbEstacion.Text;

            try
            {
                _tabla = ModelosManager.CLS.Consultas.ReporteCaja(
                    dtpFechaInicial.Value.FechaParaMySQL(), 
                    dtpFechaFinal.Value.FechaParaMySQL(), 
                    _estacion
                );

                VistasManager.REPORTES.ReporteCaja _repo = new REPORTES.ReporteCaja();
