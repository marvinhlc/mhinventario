...
{
    public partial class DocumentosCompras : Estilos.FormMantenimiento
    {
        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            try
            {
                if (_sesion.USUARIO.Kerveros(Sesion.CLS.Comandos.ELIMINAR_DOCUMENTO_COMPRAS, true))
                {
        ...
        private void btnNuevo_Click(object sender, EventArgs e)
        {
            try
            {
                DocumentosComprasGuardar _crear = new DocumentosComprasGuardar();
                _crear.doRefrescarFormularioExterno += Iniciar;
                _crear.MdiParent = this.MdiParent;
                _crear.Show();
            }
            catch (Exception _err)
            {
        ...
        private void btnEditar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DocumentosComprasGuardar _editor = new DocumentosComprasGuardar(new ModelosManager.CLS.DocumentosEncabezados(_row));
        ...
        private void dgvDetalle_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            double _precio = 0;
            double _total = 0;
            string _vencimiento = string.Empty;
            try
            {
                ...
                _vencimiento = _row.Cells["FechaVencimiento"].Value.FechaParaMySQL();
                ...
                if (_item.Existe)
                {
                    ...
                    _item.FechaVencimiento.Valor = _vencimiento;
                    if (_bd.Actualizar(_item) > 0)
                    {
                        _dtdocumentos = _documentos.DocumentosEntradas();
                        _bsdocumentos.DataSource = _dtdocumentos;
                    }
                }
            }
        ...
        private void toolStripButton1_Click(object sender, EventArgs e)
        {
            try
            {
                if (_sesion.USUARIO.Kerveros(Sesion.CLS.Comandos.ELIMINAR_DETALLE_DOCUMENTO_COMPRAS, true))
                {
        ...
        private void toolStripButton3_Click(object sender, EventArgs e)
        {
            ...
            {
                DataRow _row = dgvCatalogo.CurrentRow.ToDataRow();
                DocumentosComprasEditor _editor = new DocumentosComprasEditor(_row["IDDocumento"].TextoToEntero());
                _editor.doRefrescarFormularioExterno += Iniciar;
                _editor.ShowDialog(this);
            }
            catch (Exception _err)
