...
{
    public partial class VentaTicket : Estilos.FormMantenimiento
    {
        ...
        private CLS.Enumeraciones.TIPO_DOCUMENTO _tipo_documento = CLS.Enumeraciones.TIPO_DOCUMENTO.TICKET;
        private CLS.Enumeraciones.FORMA_PAGO _forma_pago = CLS.Enumeraciones.FORMA_PAGO.CONTADO;
        private CLS.Enumeraciones.FILTRO_DOCUMENTO _filtro_documento = CLS.Enumeraciones.FILTRO_DOCUMENTO.TIKET;
        ...
        string _cadena_comentario = string.Empty;

        public VentaTicket(CLS.Enumeraciones.TIPO_DOCUMENTO _tipo = CLS.Enumeraciones.TIPO_DOCUMENTO.TICKET)
        {
            _tipo_documento = _tipo;
            InitializeComponent();
            CargarDatos();
            Iniciar();
        }
        ...
        private void txbCodigoProducto_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                {
                    BuscarProducto(0,0,txbCodigoProducto.Text);
                }
            }
        }

        private void BuscarProducto(int _idproducto = 0, int _idbodega = 0, string _pcodigo = "")
        {
            try
            {
                DataRow _row = null;
                ...
                else
                {
                    try
                    {
                        _producto = new ModelosManager.CLS.ProductoNombres(_pcodigo);
                    }
                    catch (Exception)
                    {
                        _row = null;
                ...
                if (_producto.Existe)
                {
                    ...
                    //CARGAMOS LOS PRECIOS DE VENTA AL COMBO
                    //cbbPreciosVenta.Items.Clear();
                    //cbbPreciosVenta.Items.Add(_producto.Precio1.Valor);
                    //cbbPreciosVenta.Items.Add(_producto.Precio2.Valor);
                    //cbbPreciosVenta.Items.Add(_producto.Precio3.Valor);
                    //cbbPreciosVenta.SelectedIndex = 0;
                ...
                if (_producto.Existe)
                {
                    ...
                    {
                        //txbFechaEntrega.Text = _fecha.Fecha.ToString();
        ...
        private void AgregarItem()
        {
            ...
            try
            {
                ...
                if (_validar.Comprobar())
                {
                    ...
                    {
                        _valido = false;
                        VentaTicketFecha _calendario = new VentaTicketFecha();
                        _calendario.ShowDialog(this);
                        if (!_calendario.Fecha.EsNOE())
                        {
                            _item.FechaEntrega = _calendario.Fecha.ToString();
                            _valido = true;
                        }
                        if (_valido)
                        {
                            _valido = false;
                            VentaTicketComentario _comentario = new VentaTicketComentario(_documentos.Comentario.Valor.ToString());
                            _comentario.ShowDialog(this);
                            if (!_comentario.Comentario.EsNOE())
                            {
                                _cadena_comentario = _comentario.Comentario.ToString();
                                _valido = true;
                            }
                        }
                    }
        ...
        private void dgvDetalle_DataBindingComplete(object sender, DataGridViewBindingCompleteEventArgs e)
        {
            ...
            try
            {
                foreach (DataGridViewRow _row in dgvDetalle.Rows)
                {
                    ...
                    _total = _total + (_item.Total - _item.Descuento);
                ...
                txbTotalIVA.Text = string.Format("{0:N4}", _iva);
                txbTotalGravado.Text = string.Format("{0:N4}", _gravado);
                txbTotalExcento.Text = string.Format("{0:N4}", _excento);
                txbTotalDescuento.Text = string.Format("{0:N4}", _descuento);
                txbSubTotal.Text = string.Format("{0:N4}", _subtotal);
            }
            catch (Exception _err)
            {
                Herramientas.Log.Registrar(_err.ToString());
        ...
        private void FinalizarVenta()
        {
            ...
            try
            {
                if (_cliente.Existe == false)
                {
                    _clienteOk = false;
                    MessageBox.Show("No ha seleccionado ningún cliente", "CLIENTE", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                }

                if (dgvDetalle.ExistenFilas() > 0 && _clienteOk)
                {
                    ...
                    if (_documentos.Existe)
                    {
                        ...
                        if (!_cadena_comentario.EsNOE())
                            _nuevodoc.Comentario.Valor = _cadena_comentario;
                    }
                    else
                    {
                        ...
                        _nuevodoc.Documento.Valor = lblTipoDocumento.Text;
                        _nuevodoc.IDPersona.Valor = _cliente.IDPersona.Valor;
                        _nuevodoc.NombreTitular.Valor = _cliente.NombrePersona.Valor;
                    ...
                    if (_transac.AnexarMaestro(_nuevodoc))
                    {
                        ...
                        if (_transac.AnexarDetalle(_log))
                        {
                            ...
                            foreach (CLS.ItemVentas _item in _ventas)
                            {
                                ...
                                _detalle.Gravado.Valor = (_item.Cantidad * _item.Precio);
                                _detalle.Descuento.Valor = _item.Descuento;
                                _detalle.Total.Valor = _item.Total;
                                _detalle.FechaEntrega.Valor = _item.FechaEntrega.ToString();

                                if (_transac.AnexarDetalle(_detalle))
                                    _conteo++;
                            }

                            //CREAMOS EL REGISTRO DE LA CUENTA POR COBRAR SI LA HAY
                            //if (_saldo > 0)
                            //{
                            //    ModelosManager.CLS.PersonasPagos _pago = new ModelosManager.CLS.PersonasPagos();
                            //    _pago.IDPersona.Valor = _nuevodoc.IDPersona.Valor;
                            //    _pago.Numero.Valor = _nuevodoc.NumeroDocumento.Valor.ToString();
                            //    _pago.Fecha.Valor = _nuevodoc.FechaDocumento.Valor;
                            //    _pago.Tipo.Valor = "ABONO";
                            //    _pago.Valor.Valor = _saldo;

                            //    _transac.AnexarDetalle(_pago);
                            //}
        ...
        private void EditarPersona(bool _pactivar = true)
        {
            if (_pactivar == false)
            {
                if (txbNombreTitular.Text.NoNOE())
                {
                    ...
                    _nuevo.IDPersona.Valor = txbIDTitular.Text.TextoToEntero();
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (_pvista_previa == false)
                    {
                        ...
                        //_tabla = ModelosManager.CLS.Consultas.DocumentoImprimir(6408);
                        //_ticket = new CLS.Ticket("ticket2.txt", _tabla);
                        //_ticket.Imprimir();

                        //AGREGADP PARA OMITIR LA IMPRESION Y SOLO ABRIR LA CAJA
                        //CLS.Ticket _ticket = new CLS.Ticket();
                        //_ticket.AbrirCajon();
                    }
                    else
                    {
        ...
        private void txbPrecio_Validating(object sender, CancelEventArgs e)
        {
            ...
            {
                double _valor = 0;
                if(!double.TryParse(txbPrecio.Text, out _valor))
                {
                    e.Cancel = true;
                }
            }
        ...
        private void CapturarComentarioDocumento()
        {
            try
            {
                if (_documentos.Existe)
                {
                    ...
                    if (!_comentario.Comentario.EsNOE())
                    {
                        _documentos.Comentario.Valor = _comentario.Comentario.ToString();
                        if (_bd.Actualizar(_documentos) > 0)
                            MessageBox.Show("Se han guardado los cambios en el comentario del documento", "COMENTARIO", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
            }
            catch (Exception _err)
        ...
        private void CapturarImagenesDocumento()
        {
            try
            {
                if (_documentos.Existe)
                {
                    VentaTicketImagenes _galeria = new VentaTicketImagenes(_documentos.IDDocumento.Valor.TextoToEntero());
                    _galeria.ShowDialog(this);
                }
            }
            catch (Exception _err)
            {
        ...
        {
            CapturarComentarioDocumento();
        }

        private void btnImagenes_Click(object sender, EventArgs e)
        {
            CapturarImagenesDocumento();
        }
        ...
        private void AplicarDescuentoVenta()
        {
            bool _aplicar = false;
            double _total = 0;
            double _iva = 0;
            ...
            try
            {
                _aplicar = ckbAplicarDescuento.Checked;
                foreach (CLS.ItemVentas _item in _ventas)
                {
                    //APLICAMOS SI HAY DESCUENTO O NO
                    _item.Aplicar_descuento = _aplicar;
                    _item.Calcular();
