...
{
    public partial class DocumentosSalidasEditor : Estilos.FormMantenimiento
    {
        ...
        public DocumentosSalidasEditor(int _iddocumento = 0)
        {
        ...
        private void Iniciar()
        {
            try
            {
                ...
                cbbPersonas.ComboBox.DataSource = _bspersonas;
                cbbPersonas.ComboBox.DisplayMember = "NombrePersona";
                cbbPersonas.ComboBox.ValueMember = "IDPersona";
                cbbPersonas.ComboBox.SelectedIndex = 0;
        ...
        private void dgvCatalogo_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(_row.Cells["Cantidad"].Value.ToString()))
                {
                    _valor = 0;
                    _row.Cells["Cantidad"].Value = _valor;
                }
            }
            catch (Exception _err)
            {
            ...
            try
            {
                ...
                if (dgvCatalogo.ExistenFilasEditadas("Cantidad") > 0)
                {
                    if (_documento.Existe)
                    {
                        ...
                        DocumentosSalidasGuardar _guardar = new DocumentosSalidasGuardar(_documento);
                    ...
                    else
                    {
                        ...
                        DocumentosSalidasGuardar _guardar = new DocumentosSalidasGuardar(_idpersona);
                        _guardar.doRefrescarFormularioExterno += Guardar;
                        _guardar.ShowDialog(this);
                        if (_guardar.Ok)
                        {
            ...
            try
            {
                ...
                if (_ok == true)
                {
                    ...
                    foreach (DataGridViewRow _row in _grows)
                    {
                        ...
                        _item.FormatoVenta.Valor = _row.Cells["FormatoVenta"].Value.ToString();
        ...
        private void ImprimirDocumento(int _piddocumento)
        {
            try
            {
                DataTable _dtrepo = ModelosManager.CLS.Consultas.HojaSalida(_piddocumento);

                REPORTES.HojaSalidas _hoja = new REPORTES.HojaSalidas();
                _hoja.SetDataSource(_dtrepo);

                VistasManager.UI.VistaPrevia _vp = new VistaPrevia(_hoja);
                _vp.Show();
            }
            catch (Exception _err)
            {
        ...
        private void btnNuevoProducto_Click(object sender, EventArgs e)
        {
            try
            {
                int _pidproveedor = cbbPersonas.ComboBox.SelectedValue.TextoToEntero();
