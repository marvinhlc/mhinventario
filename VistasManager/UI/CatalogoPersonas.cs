...
{
    public partial class CatalogoPersonas : Estilos.FormMantenimiento
    {
        ...
        private void FiltrarPersonas()
        {
            try
            {
                if (_filtro == CLS.Enumeraciones.TIPO_PERSONA.PROVEEDORES)
                {
        ...
        {
            _filtro = CLS.Enumeraciones.TIPO_PERSONA.PROVEEDORES;
            FiltrarPersonas();
        }

        private void radioButton2_CheckedChanged(object sender, EventArgs e)
        {
            _filtro = CLS.Enumeraciones.TIPO_PERSONA.CLIENTES;
            FiltrarPersonas();
        }
        ...
        private void btnNuevo_Click(object sender, EventArgs e)
        {
            try
            {
                CatalogoPersonasEditor _editor = new CatalogoPersonasEditor(0, _filtro);
        ...
        private void btnEditar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                CatalogoPersonasEditor _editor = new CatalogoPersonasEditor(_row["IDPersona"].TextoToEntero(), _filtro);
                _editor.doRefrescarFormularioExterno += Iniciar;
                _editor.ShowDialog(this);
            }
            catch (Exception _err)
