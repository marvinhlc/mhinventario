...
{
    public partial class DocumentosComprasEditor : Estilos.FormMantenimiento
    {
        ...
        //NECESARIO PARA COMPORTAMIENTO DE TECLAS
        DataGridViewCell _curret_cell = null;

        public DocumentosComprasEditor(int _iddocumento = 0)
        {
        ...
        private void CargarCatalogoProductos()
        {
            try
            {
                ...
                _dtproductos = ModelosManager.CLS.Consultas.HojaEntradaProductos();
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dgvCatalogo.ExistenFilasEditadas("Cantidad") > 0)
                {
                    if (_documento.Existe)
                    {
                        if (cbbProveedores.ComboBox.SelectedValue.TextoToEntero() != _documento.IDPersona.Valor.TextoToEntero())
                        {
                            if(MessageBox.Show("¿Desea cambiar el proveedor del documento que está editando?","GUARDAR",MessageBoxButtons.YesNo,MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes)
                            {
                                _documento.IDPersona.Valor = cbbProveedores.ComboBox.SelectedValue.TextoToEntero();
                                _documento.NombreTitular.Valor = cbbProveedores.ComboBox.Text;
                            }
                        }
            ...
            try
            {
                ...
                if (_ok == true)
                {
                    ...
                    foreach (DataGridViewRow _row in _grows)
                    {
                        ...
                        _item.UltimoCosto.Valor = _row.Cells["UltimoCosto"].Value;
                        _item.IDBodega.Valor = cbbBodegas.ComboBox.SelectedValue.TextoToEntero();
                        _item.NombreBodega.Valor = cbbBodegas.ComboBox.Text;
                        _item.FechaVencimiento.Valor = _row.Cells["FechaVencimiento"].Value.FechaParaMySQL();
        ...
        private void btnNuevoProducto_Click(object sender, EventArgs e)
        {
            try
            {
                int _pidproveedor = cbbProveedores.ComboBox.SelectedValue.TextoToEntero();
        ...
        private void dgvCatalogo_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    dgvCatalogo.ToNextColumna(_curret_cell);
                    //_curret_cell = null;
                }
            }
            catch (Exception _err)
        ...
        private void dgvCatalogo_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                _curret_cell = dgvCatalogo.CurrentCell;
                ...
                //{
                //    _nuevocosto = CLS.ExtensionesLocales.GuardarUltimoCosto(dgvCatalogo, _row);
                //    if (_nuevocosto > 0)
                //        _row.Cells["UltimoCosto"].Value = _nuevocosto;
                //}
                KeyEventArgs _keydown = new KeyEventArgs(Keys.Enter);
                dgvCatalogo_KeyDown(dgvCatalogo, _keydown);
            }
            catch (Exception _err)
            {
                Herramientas.Log.Registrar(_err.ToString());
        ...
        {
            if (_curret_cell != null)
            {
                dgvCatalogo.CurrentCell = _curret_cell;
                KeyEventArgs _keydown = new KeyEventArgs(Keys.Enter);
                dgvCatalogo_KeyDown(dgvCatalogo, _keydown);
                _curret_cell = null;
            }
